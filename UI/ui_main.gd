...
func _ready() -> void:
	$FilterForLevelTransition.visible = true
	$FilterForLevelTransition.modulate = Color(0,0,0,0)
	$FilterForLevelTransition.mouse_filter = Control.MOUSE_FILTER_IGNORE
...
func switchLevel(_lvlName:String):
	...
	await tween.tween_property($FilterForLevelTransition,"modulate",Color(0,0,0,1),0.3).finished
	get_tree().change_scene_to_file(_lvlName)
	var tween2 = get_tree().create_tween()
	tween2.tween_property($FilterForLevelTransition,"modulate",Color(0,0,0,0),0.3)
